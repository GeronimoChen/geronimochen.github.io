...
*/
    ...
    *You need to pass the updated image urls to config variable through javascript or php
    ...
    var GenerateTabbedHTMLfromArray = function (optimizedImages, originalImages, thumbimages, dsbrowblocklabel) {
        var check_tabbedimages = tabbedis_array(optimizedImages); //Checking is array
        ...
        if (check_tabbedimages) {
            ...
            for (var i = 0; i < originalImages.length; i++) {
                ...
                tabbedimages_html += '<input id="' + i + 'dsassets" type="radio" name="dsimagebrowserradio" value="1" onclick="SetURLtoCKeditor(\'' + (optimizedImages ? optimizedImages[i] : originalImages[i]) + '\')">';
                tabbedimages_html += '<div class="buttons"><span class="image-edit" data-image="' + originalImages[i] + '">Edit</span><span class="image-delete" data-imageInContent="' + (optimizedImages ? optimizedImages[i] : originalImages[i])+'" data-image="' + originalImages[i] + '">Delete</span></div>';
    ...
    var GenerateTabbedbrowserHTML = function () {
        ...
        if (CKEDITOR.config.tabbedimages != undefined && CKEDITOR.config.tabbedthumbimages != undefined) {
            tabbedimages_html = GenerateTabbedHTMLfromArray(CKEDITOR.config.tabbedoptimizedimages,CKEDITOR.config.tabbedimages, CKEDITOR.config.tabbedthumbimages, '');
        }
        else {
            tabbedimages_html = '<div>No images available.</div>';
        }
    ...
    var ReplaceDSbrowserHTML = function () {
        ...
        if (!CKEDITOR || !CKEDITOR.currentInstance) return;
        let galleryContainerId = CKEDITOR.currentInstance.id + '_dsbrowserfullcontainer';
        if (!document.getElementById(galleryContainerId)) return;

        var tabbedimages_html = GenerateTabbedbrowserHTML();
        document.getElementById(galleryContainerId).innerHTML = tabbedimages_html;
    };
    ...
    //var ReplaceVideoBrowserHTML = function () {
    //    var tabbedimages_html = GenerateTabbedbrowserHTML();
    ...
    //};
    //$(document).on("updateVideoGallery", function () {
    //    ReplaceVideoBrowserHTML();
    //});
            ...
            init: function (editor) {
                ...
                CKEDITOR.on('dialogDefinition', function (ev) {
                    ...
                    if (dialogName === 'image2') {
                        ...
                        if (!dialogDefinition.contents.find(tab => tab.id === "TabbedImageBrowser"))
                                ...
                                elements: [
                                    {
                                        ...
                                        children: [
                                            {
                                                type: 'html',
                                                html: '<div id="'+CKEDITOR.currentInstance.id+'_dsbrowserfullcontainer">' + tabbedimages_html + '</div>'
                                            }
                                        ]
                                    },
                                    {
                        ...
                        infoTab.remove('width'); // Remove width element from Info tab
                        infoTab.remove('height'); // Remove height element from Info tab
                        infoTab.remove('lock'); // Remove height element from Info tab
                    }
...
var SetURLtoCKeditor = function (imgurl) {
    var dialog = CKEDITOR.dialog.getCurrent();
    dialog.setValueOf('info', 'src', imgurl);  // Populates the URL field in the Links dialogu
    //dialog.setValueOf('info', 'width', '');
    //dialog.setValueOf('info', 'height', '');
};
